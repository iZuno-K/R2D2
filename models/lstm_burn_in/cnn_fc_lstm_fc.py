...
def _cnn_to_fc_to_lstm_to_fc(convs, hiddens, dueling, inpt, lstm_inpt, num_actions, scope, max_length=None,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("fc1"):
            fc_out = tf.layers.flatten(conv_out)
            fc_out = tf.layers.dense(fc_out, units=256, activation=None)
            if layer_norm:
                fc_out = tf.contrib.layers.layer_norm(fc_out, center=True, scale=True)
        ...
        with tf.variable_scope("lstm"):
            ...
            if max_length is None:  # one step execution
                ...
                lstm_out, lstm_state = tf.nn.dynamic_rnn(cell=lstm, inputs=tf.expand_dims(fc_out, 1), initial_state=lstm_state)
                lstm_out = tf.squeeze(lstm_out, 1)
            else:  # sequential inputs (training phase)
                state_dim = fc_out.shape[1]
                fc_out = tf.reshape(fc_out, (-1, max_length, state_dim))  # (batch, max_length, state_dim)
                lstm_out, lstm_state = tf.nn.dynamic_rnn(cell=lstm, inputs=fc_out,
...
def cnn_to_fc_to_lstm_to_fc(convs, hiddens, dueling=False, layer_norm=False):
